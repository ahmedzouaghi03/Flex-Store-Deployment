...
// SVG is intentionally excluded — SVGs served from the same origin can
// carry inline <script> tags (stored XSS).
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validation = validateUpload(file, ALLOWED_LOGO_TYPES, "logo");
    if (!validation.ok) {
